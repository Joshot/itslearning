...
<div class="flex w-full h-full flex-col md:flex-row">
    ...
    <div class="w-full md:w-1/4 bg-white flex flex-col justify-center items-center p-8">
        ...
        <h1 style="color: #54606b" class="font-bold text-2xl mb-6">Sign In Lecturer</h1>
        ...
        <form action="{{ route('lecturer.login.process') }}" method="POST" class="w-full" onsubmit="return validateForm()">
            @csrf
            <div class="mb-4">
                <label for="email" style="color: #54606b" class="block">Email Lecturer</label>
                <input type="email" name="email" id="email" class="w-full p-2 border border-gray-300 rounded mt-1" value="{{ old('email') }}" required>
            </div>
            <div class="mb-4">
...
<script>
    function validateForm() {
        ...
        var emailRegex = /^[a-zA-Z]+@lecturer\.umn\.ac\.id$/;
        if (!email) {
        ...
        if (!emailRegex.test(email)) {
            errorMessage.innerText = "Email harus menggunakan format nama@lecturer.umn.ac.id!";
            errorMessage.classList.remove("hidden");
            return false;
        }
        if (!password) {
